...
int speed_truck = 15;// Circa 15 Km/h
...
execute {
	...
	//cambiamento per grafo incompleto considerando di riceve una matrice simmetrica nei dati
	for(var ii in N) {
		for(var jj in N) {
			  if(dist[ii][jj] <= speed_truck*TS) {
			  	A1.add(ii, jj, E_truck*Opl.ceil(dist[ii][jj]), Opl.ceil(dist[ii][jj])); 
				A1.add(jj,ii, E_truck*Opl.ceil(dist[ii][jj]), Opl.ceil(dist[ii][jj])); 
				// Senza Opl.ceil() -> distanze precise ma il modello è molto lento
 			  }		
 //			}
		}
	}		
...
subject to {
	...
	forall(ti in Tdim) {
		ctk2:
		sum(a in A1: a.j==1) z[a][ti+1] >= 1 + (k[ti] - Nc);
		
		ctk3:
		sum(a in A1:a.i!=1 && a.j==1) z[a][ti+1] <= 1 - (k[ti] - Nc);
	}
	...
	//nuovo vincolo
        forall(a2 in A2, di in D, ti in T){
        x[a2][di][ti] <= (1- sum(a in A1: a.j!=a.i)z[a][ti]);
        }
...
//vincolo definizione variabile
	forall (di in D, ti in T){
	    m2[di][ti]*M >= (e[di][ti] - 0.5*(Emax-Emin));
   }	    
